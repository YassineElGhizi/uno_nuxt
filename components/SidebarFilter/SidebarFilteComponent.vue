...
export default {
  ...
  methods :{
    ...
    handleChnage(event){
      if (this.$props.name == 'Marques'){
        let brand_id = parseInt(event.target.value)
        this.$store.dispatch('search/setFilterBrand' , brand_id)
      }
      if (this.$props.name == 'Categories'){
        let category_id = parseInt(event.target.value)
        this.$store.dispatch('search/setFilterCategory', category_id)
      }
    },
  },
}
